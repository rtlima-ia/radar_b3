    ...
    <body class="bg-gradient-to-b from-[#ff7a00] to-[#b34f00] text-white min-h-screen flex flex-col items-center justify-between font-sans p-4 relative overflow-x-hidden selection:bg-[#ff8c21] selection:text-white">
        ...
        <script>
            ...
            formB3.addEventListener('submit', async (e) => {
                ...
                const ativoSelecionado = inputAtivo.value.toUpperCase();
                const condicaoTexto = selectCondicao.options[selectCondicao.selectedIndex].text;
                    ...
                    if (dados.status === "sucesso") {
                        ...
                        feedback.innerHTML = `
                            ...
                            <div class="text-sm text-orange-100">
                                <p>Ativo: <span class="font-bold text-white">${ativoSelecionado}</span></p>
                                <p>Alvo: <span class="font-bold text-white">R$ ${precoLimpoParaEnvio.toFixed(2)}</span></p>
                                <p>Regra: <span class="font-bold text-white">${condicaoTexto}</span></p>
                                <p class="mt-2 pt-2 border-t border-white/10">Destino: <span class="text-white underline font-bold">${dados.email}</span></p>
                            </div>
                        `;
                        formB3.reset();
